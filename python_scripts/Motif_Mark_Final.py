...
fasta_df = open('fasta_sample.txt.txt')
...
surface = cairo.SVGSurface("Leslie_plot.svg", entry_len+(x_coord*2), (nmbr_lines*50)+50)
...
for sequence_lines in fasta_list:
    ...
    if sequence_lines.startswith(">"): 
        ...
        context.move_to(x_cord, y_cord-15)
    ...
    if not sequence_lines.startswith(">"): 
        ...
        context.set_line_width(0.5)
        context.move_to(x_cord, y_cord)
        context.line_to(x_cord+len(sequence_lines),y_cord)
        context.stroke()
